...
\section{Unsupervised Learning}
  ...
  on the right I present the average \emph{three}, on the left the eigenvalues of the whole
  dataset, scaled over their sum. The first components explain most of the variance:
  just 12 explain 63\%, and with 50 it adds up to 90\%.  Therefore, using 50 
  components or more should already deliver a good reconstruction of the data.
  \autoref{fig:digits_rec} confirms it: with more than 50 components
  reconstruction  \emph{MSE} is already under \num{0.1}.  On the other hand, 
  on random data variance spreads more uniformly across the components, as 
  the green line in \autoref{fig:first_task} shows.
  It takes more components to have both decent reconstruction error and summed 
  variance. For example, 12 components only explain 34\% of the variance, and
  deliver \num{0.8} $MSE$ on the reconstruction. Finally, both digits and random
  data show a strong correlation between the sum of variance of the last k left-out
  components and the reconstruction error, displayed in \autoref{fig:regression}. 
  The shape of the curve suggests that the relationship between them is logarithmic,
  not linear.
  ...
  \begin{figure}[h]
    ...
    plotting the eigenvalues (left). The digits eigenvalues have been compared
  ...
  \begin{figure}[h]
    ...
    \end{subfigure}
    \begin{subfigure}[t]{0.3\linewidth}
      \includegraphics[width=1\linewidth, height=4.65cm]{./lab3/PCA/rec_vs_invsum_digits.png}
      \caption{Digits dataset}
      \label{fig:digits_rec}
    \end{subfigure}
    \begin{subfigure}[t]{0.3\linewidth}
      \includegraphics[width=1\linewidth, height=4.65cm]{./lab3/PCA/regression_digits.png}
